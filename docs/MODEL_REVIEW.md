...
为避免只看风格成片判断音准，端到端结果现在额外渲染 `melody.wav`，并通过 `melody_audio_url` 返回 canonical MIDI 的试听副本。线上 job `64595c59ed27486687168b6c65a54904` 的 `renderers.melody` 为 `fluidsynth-soundfont`，原始 MIDI、Funk、Lofi 三路音频都可以直接播放对照。
...
又对 Spotify Basic Pitch 做了离线参数扫描（`minimum_note_length=127.7–380ms`、多组 onset/frame threshold）。默认参数在同一录音上产出 23 个碎音；提高阈值虽然能降到 14–15 个，但会吞掉重复音、错配起音并丢失第二句，无法稳定复现当前 DSP 的 14 音序列。因此 Basic Pitch 暂不替换生产基线。

为验证“模型能否从整段旋律判断音乐意图”，又加入了不带曲名的调性候选：以结尾音推断大调，把半音边界音映射到最近级内音，同时保留原始人声音质感和节奏。4 次随机候选顺序复测中，模型选择 `dsp` 两次、`smoothed` 一次、`diatonic` 一次，没有形成稳定偏好；因此调性候选暂不自动应用，避免把真实离调哼唱强行改成错误标准音。

最后尝试让 Omni 只输出相邻音的半音间隔（不输出绝对 MIDI）。Plus 和 Flash 都能返回 14 音、两句，但同一 Prompt 的间隔序列分别为 `[0,2,0,-2,-2,-2,4,...]` 与 `[0,-1,0,2,-3,4,-2,...]`，彼此不一致且与 DSP 实测轮廓不符。说明“减少输出格式复杂度”仍不能把通用音频理解变成可靠的逐音音高校正。
