...
Worker 的第一阶段改动只允许调整官方模型调用方式：以 Quick Start 的参数、Prosody 模板输入和负向分支为准，不改模型权重或 DiT 内部 forward。代码提交到 `main` 后由常驻 daemon 自动拉取、等待任务排空、重启 Worker 并做健康检查；daemon 脚本本身不随这次改动修改。
